...
var url = require('url');
var CapturePlayStrategy = require('passport-captureplay/strategy');


vows.describe('CapturePlayStrategy').addBatch({
  
  'strategy': {
    topic: function() {
      return new CapturePlayStrategy({
        clientID: 'ABC123',
        clientSecret: 'secret'
      },
      function() {});
    ...
    'should be named captureplay': function (strategy) {
      assert.equal(strategy.name, 'captureplay');
    },
  },
  
  'strategy when redirecting for authorization': {
    topic: function () {
      var strategy = new CapturePlayStrategy({
        clientID: 'ABC123',
        clientSecret: 'secret'
      });
  ...
  'strategy when loading user profile': {
    topic: function() {
      ...
      strategy._oauth2.getProtectedResource = function(url, accessToken, callback) {
        if (url == 'https://graph.captureplay.com/me') {
          var body = '{"id":"500308595","name":"Jared Hanson","first_name":"Jared","last_name":"Hanson","link":"http:\\/\\/www.captureplay.com\\/jaredhanson","username":"jaredhanson","gender":"male","email":"jaredhanson\\u0040example.com"}';
    ...
    'when told to load user profile': {
      ...
      'should load profile' : function(err, profile) {
        assert.equal(profile.provider, 'captureplay');
        ...
        assert.equal(profile.profileUrl, 'http://www.captureplay.com/jaredhanson');
  ...
  'strategy when loading user profile with profileURL option': {
    topic: function() {
      var strategy = new CapturePlayStrategy({
        clientID: 'ABC123',
        clientSecret: 'secret',
        profileURL: 'https://graph.captureplay.com/me?fields=id,username'
      },
      function() {});
      
      // mock
      strategy._oauth2.getProtectedResource = function(url, accessToken, callback) {
        if (url == 'https://graph.captureplay.com/me?fields=id,username') {
          var body = '{"id":"500308595","name":"Jared Hanson","first_name":"Jared","last_name":"Hanson","link":"http:\\/\\/www.captureplay.com\\/jaredhanson","username":"jaredhanson","gender":"male","email":"jaredhanson\\u0040example.com"}';
          callback(null, body, undefined);
        } else {
  ...
  'strategy when loading user profile with mapped profile fields': {
    topic: function() {
      ...
      // mock
      strategy._oauth2.getProtectedResource = function(url, accessToken, callback) {
        if (url == 'https://graph.captureplay.com/me?fields=id,username,name,last_name,first_name,middle_name,gender,link,email,picture') {
          var body = '{"id":"500308595","name":"Jared Hanson","first_name":"Jared","last_name":"Hanson","link":"http:\\/\\/www.captureplay.com\\/jaredhanson","username":"jaredhanson","gender":"male","email":"jaredhanson\\u0040example.com"}';
          callback(null, body, undefined);
        } else {
          callback(new Error('Incorrect user profile URL: ' + url));
  ...
  'strategy when loading user profile with id and photos': {
    topic: function() {
      ...
      // mock
      strategy._oauth2.getProtectedResource = function(url, accessToken, callback) {
        if (url == 'https://graph.captureplay.com/me?fields=id,picture') {
